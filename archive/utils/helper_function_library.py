...
class XRD_ScanAnalyzer:
    ...
    def make_saving_string(self,method):
        ...
        reflectionstr = 'r'+self.reflection.replace(')','').replace('(','').replace('-','m').replace(',','')
    ...
    def plot_scan_image(self,img,cbarlabel='Intensity',vmin=None,vmax=None,save=True):
        """
        ...
        scalebar = ScaleBar(self.scale,"um",box_alpha=0,color='white',location='lower right')
        cbar=plt.colorbar()
    ...
    def image_roi_shift(self,axis='x',vmin=None,vmax=None,threshold=0,normed_prof=[],save=True):
        ...
        self.normed_prof = normed_prof
        img = [] 
        img_std = []
        
        for x in tqdm(self.roi_data):
            summed = np.sum(x,axis_int)
            if np.sum(summed) >= threshold:
        ...
        self.plot_scan_image(img,'SHIFT'+axis,vmin,vmax,save)
        self.shift_img = img
