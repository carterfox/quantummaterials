...
end FlxRioHbDecMac2xOc;

architecture RTL of FlxRioHbDecMac2xOc is
  ...
  constant kIsLastMac : boolean := kMacIndex=(CalcNumOfMacs(kCyclesPerInput,kInputSamplesPerCycle)-1);

  constant kCycleCountLength : integer := CalcCycleCountLength(kCyclesPerInput,kInputSamplesPerCycle);
  ...
  begin
      if(kInputSamplesPerCycle > 1) then
        ...
        NumNonZeroCoeffPerSegment := (kNumMacsPerFilter2xOc*kOverclockingFactor)/kInputSamplesPerCycle;
        -- Calculate the coefficient offset within a segment
        OffsetWithinSegment := (kMacIndex mod NumNonZeroCoeffPerSegment)*kNumCalcCycles*kInputSamplesPerCycle;
        -- Calculate the offset of the segment
        SegmentOffset := (kMacIndex / NumNonZeroCoeffPerSegment)*kOverclockingFactor;
        -- Calculate the base coefficient index for cycle zero
        ReturnVal  := OffsetWithinSegment + SegmentOffset;

        -- Add kInputSamplesPerCycle if it is cycle one
        ReturnVal := ReturnVal + (kInputSamplesPerCycle * CycleCount);
        ...
        -- We multiply be two becuase we don't calculate the zero coefficients
        ReturnVal := kMacIndex * kNumCalcCycles * 2;
        -- Adjust for this particulare cycle count
        -- We multiply be two becuase we don't calculate the zero coefficients
  ...
  -- This function calculates an array containing the subset of kHbDecCoefficients used on this MAC.
  -- The creation of this array helps the tools understand that each mac uses only a subset of
  -- kHbDecCoefficients. Implementing cCoefficient without this function confused the tools and a ROM
  -- with the whole filter was inferred in a Block RAM for each MAC.
  function CalcCoeffForThisMac return HbDecCoefficients_t is
      variable ReturnVal : HbDecCoefficients_t(kNumCalcCycles - 1 downto 0);
  begin
      for i in kNumCalcCycles - 1 downto 0 loop
        ReturnVal(i) := kHbDecCoefficients(CalcForCoeffIndex(i));
      end loop;
      return ReturnVal;
  end function CalcCoeffForThisMac;
  ...
  begin
      if(kInputSamplesPerCycle > 1) then
        -- Calculate the adjustment to the cofficient index based on the filter index.  This formula was derived
        -- by analyzing address patterns for each kInputSamplesPerCycle in a spreadsheet.
        ReturnVal := CoeffIndex + (kInputSamplesPerCycle - 1) - (kDecimationFactor * kFilterIndex);
        -- Divide by kInputSamplesPerCycle to get the final SRL address.
        ReturnVal := ReturnVal / kInputSamplesPerCycle;
      else
  ...
  constant kRevSrlAddrCycleZero : integer := CalcSrlAddr(kHbDecCoefficients'high - CalcForCoeffIndex(0));
  ...
  constant kMacCoefficients : HbDecCoefficients_t(kNumCalcCycles - 1 downto 0)  := CalcCoeffForThisMac;
  signal cCoefficient : signed(17 downto 0) := kMacCoefficients(0);
  ...
  -- Synthesis errors happened when Vivado optimized and retimed cCycleCount. We add a
  -- keep to cCycleCount to avoid running into that issues again.
  attribute keep of cCycleCount: signal is "true";
...
begin
  ...
  -- The cResetPipe signal insures the counter starts up safely.
  process(Clk)
  begin
    if rising_edge(Clk) then
      if(cDataInValid or cResetPipe) then
  ...
  begin
    if rising_edge(Clk) then
      cCoefficient <= kMacCoefficients(to_integer(cCycleCount));
    end if;
  end process;
