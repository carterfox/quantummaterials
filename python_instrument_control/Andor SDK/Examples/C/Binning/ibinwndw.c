//------------------------------------------------------------------------------
//  PROJECT:		32-bit Driver Example Code ---- Image Acquisition with Binning
//
...
//							sub image acquisition with binning and display 3 tracks from it.
...
int yHeight=605;   // height of application window passed to common.c
//******************************************************************************
...
BOOL CreateWindows(void)
{
    ...
    wsprintf(aBuffer2,"%d",gblYPixels);
    ...
    ebStatus=CreateWindow("EDIT","",
    											WS_CHILD|WS_VISIBLE|WS_BORDER|ES_LEFT|ES_AUTOVSCROLL|ES_MULTILINE,
													10,420,600,150,hwnd,0,hInstance,NULL);
    pbStart=CreateWindow("BUTTON","Start Acq",
    											WS_CHILD|WS_VISIBLE|WS_BORDER|BS_PUSHBUTTON,
													10,360,90,30,hwnd,0,hInstance,NULL);
    pbAbort=CreateWindow("BUTTON","Abort Acq",
    											WS_CHILD|WS_VISIBLE|WS_BORDER|BS_PUSHBUTTON,
													120,360,90,30,hwnd,0,hInstance,NULL);
    pbClose=CreateWindow("BUTTON","Close",
    											WS_CHILD|WS_VISIBLE|WS_BORDER|BS_PUSHBUTTON,
													230,360,90,30,hwnd,0,hInstance,NULL);
    stStatus=CreateWindow("STATIC","Status",
    											WS_CHILD|WS_VISIBLE|SS_LEFT,
													10,400,60,18,hwnd,0,hInstance,NULL);

    ebHStart=CreateWindow("EDIT", "1",
    											WS_CHILD|WS_VISIBLE|WS_BORDER|ES_LEFT,
  										 		90,240,50,20,hwnd,0,hInstance,NULL);
    ebHEnd=CreateWindow("EDIT", aBuffer,
    											WS_CHILD|WS_VISIBLE|WS_BORDER|ES_LEFT,
  										 		90,260,50,20,hwnd,0,hInstance,NULL);
    ebVStart=CreateWindow("EDIT", "1",
    											WS_CHILD|WS_VISIBLE|WS_BORDER|ES_LEFT,
  										 		90,290,50,20,hwnd,0,hInstance,NULL);
    ebVEnd=CreateWindow("EDIT", aBuffer2,
    											WS_CHILD|WS_VISIBLE|WS_BORDER|ES_LEFT,
  										 		90,310,50,20,hwnd,0,hInstance,NULL);
		ebHBin=CreateWindow("EDIT", "1",
    											WS_CHILD|WS_VISIBLE|WS_BORDER|ES_LEFT,
  										 		270,250,50,20,hwnd,0,hInstance,NULL);
    ebVBin=CreateWindow("EDIT", "1",
    											WS_CHILD|WS_VISIBLE|WS_BORDER|ES_LEFT,
  										 		270,300,50,20,hwnd,0,hInstance,NULL);

    stHStart=CreateWindow("STATIC","Horiz. Start",
    											WS_CHILD|WS_VISIBLE|SS_LEFT,
													10,240,80,20,hwnd,0,hInstance,NULL);
    stHEnd=CreateWindow("STATIC","Horiz. End",
    											WS_CHILD|WS_VISIBLE|SS_LEFT,
													10,260,80,20,hwnd,0,hInstance,NULL);
    stVStart=CreateWindow("STATIC","Vert. Start",
    											WS_CHILD|WS_VISIBLE|SS_LEFT,
													10,290,80,20,hwnd,0,hInstance,NULL);
    stVEnd=CreateWindow("STATIC","Vert. End",
    											WS_CHILD|WS_VISIBLE|SS_LEFT,
													10,310,80,20,hwnd,0,hInstance,NULL);
    stHBin=CreateWindow("STATIC","Horiz. Binning",
    											WS_CHILD|WS_VISIBLE|SS_LEFT,
													160,250,100,20,hwnd,0,hInstance,NULL);
    stVBin=CreateWindow("STATIC","Vert. Binning",
    											WS_CHILD|WS_VISIBLE|SS_LEFT,
													160,300,100,20,hwnd,0,hInstance,NULL);
...
void SetWindowsToDefault(char aInitializeString[256])
{
  ...
  // Fill in default exposure time
  //wsprintf(aBuffer,"1");
  //SendMessage(ebHStart, WM_SETTEXT, 0, (LPARAM)(LPSTR)aBuffer);
  ...
  vstart=1;
  vend=gblYPixels;
  hDim = (hend-hstart+1)/hbin;
  vDim = (vend-vstart+1)/vbin;
}
...
void SetSystem(void)
{
  ...
  //Get Image setup parameters ie.subimage coords and binning sizes
  GetWindowText(ebHStart,aBuffer2,10);
  hstart=atof(aBuffer2);

  GetWindowText(ebHEnd,aBuffer2, 10);
  hend=atof(aBuffer2);

  GetWindowText(ebVStart,aBuffer2, 10);
  vstart=atof(aBuffer2);

  GetWindowText(ebVEnd,aBuffer2, 10);
  vend=atof(aBuffer2);

  GetWindowText(ebHBin,aBuffer2, 10);
  hbin=atof(aBuffer2);

  GetWindowText(ebVBin,aBuffer2, 10);
  vbin=atof(aBuffer2);
  ...
  if(hstart>hend || vstart>vend){
    strcat(aBuffer,"Image start positions must not be greater than end positions\r\n");
    gblData=FALSE;
  }
  if(hstart<1 || hend>gblXPixels){
    strcat(aBuffer,"Image horiz. positions must be >0 and <detector width\r\n");
    gblData=FALSE;
  }
  if(vstart<1 || vend>gblYPixels){
  	strcat(aBuffer,"Image vert. positions must be >0 and <detector height\r\n");
    gblData=FALSE;
  }
  if((hend-hstart+1)%hbin!=0){
  	strcat(aBuffer,"Image width must be a multiple of Horizontal Binning\r\n");
    gblData=FALSE;
  }
  if((vend-vstart+1)%vbin!=0){
    strcat(aBuffer,"Image height must be a multiple of Vertical Binning\r\n");
    gblData=FALSE;
  }
  ...
  if(errorValue!=20002){
  	strcat(aBuffer,"Set Image Error\r\n");
    gblData=FALSE;
  }
  else{
...
void UpdateDialogWindows(void)
{
  ...
  UpdateWindow(ebHStart);
  UpdateWindow(ebVStart);
  UpdateWindow(ebHEnd);
  UpdateWindow(ebVEnd);
  UpdateWindow(ebVBin);
  UpdateWindow(ebHBin);
  UpdateWindow(stHStart);
  UpdateWindow(stVStart);
  UpdateWindow(stHEnd);
  UpdateWindow(stVEnd);
  UpdateWindow(stVBin);
  UpdateWindow(stHBin);
...
BOOL AcquireImageData(void)
{
  ...
  if(!gblData){														  // If there is no data the acq has
    ...
    wsprintf(aBuffer2,"Image acquired\r\n\r\n");
    strcat(aBuffer,aBuffer2);
    ...
    if(hbin==1)
    	wsprintf(aBuffer2,"\r\nNo Horizontal Binning\r\n");
			...
			wsprintf(aBuffer2,"\r\n%d Data Values per Track\r\n", hDim);
    strcat(aBuffer,aBuffer2);
    if(vbin==1)
      wsprintf(aBuffer2,"No Vertical Binning\r\n");
    else
    	wsprintf(aBuffer2,"%d Binned Tracks\r\n", vDim);
    strcat(aBuffer,aBuffer2);
  }
  SendMessage(ebStatus,WM_SETTEXT,0,(LPARAM)(LPSTR)aBuffer);
...
int AllocateBuffers(void)
{
	...
	size=hDim * vDim;  // Needs to hold full image
...
void PaintImage(long maxValue, long minValue, int Start)
{
  ...
  if (lvRunning == FALSE) {// if the screensaver is not running, plot data.
    ...
    for (i = 0; i < height; i++) {
      ...
      for (j = 0; j < width; j++) {
        x = (int)(j * xscale);
        dTemp = ceil(yscale * (pImageArray[Start + x + z * hDim] - minValue));
        if (dTemp < 0) iTemp = 0;
