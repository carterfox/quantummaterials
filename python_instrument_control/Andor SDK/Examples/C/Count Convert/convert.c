//------------------------------------------------------------------------------
//  PROJECT:		32-bit Driver Example Code ---- Count Convert
//                                              
...
BOOL CreateWindows(void)
{
  ...
  stCountConvert=CreateWindow("STATIC","Count Convert Mode:",
                        WS_CHILD|WS_VISIBLE|SS_LEFT,
                        10,110,180,20,hwnd,0,hInstance,NULL);
  cbCountConvert=CreateWindow("COMBOBOX","",
                        WS_CHILD|WS_VISIBLE|CBS_DROPDOWNLIST,
                        210,110,90,80,hwnd,0,hInstance,NULL);
  stWavelength=CreateWindow("STATIC","Wavelength (nm):",
                        WS_CHILD|WS_VISIBLE|SS_LEFT,
                        10,140,160,20,hwnd,0,hInstance,NULL);
  ebWavelength=CreateWindow("EDIT","",
                        WS_CHILD|WS_VISIBLE|WS_BORDER|ES_LEFT,
...
void SetWindowsToDefault(char aInitializeString[256])
{
  ...
  // Add Count Convert options to combobox
  wsprintf(aBuffer,"Counts");
  SendMessage(cbCountConvert, CB_ADDSTRING, 0, (LPARAM)(LPSTR)aBuffer);
  wsprintf(aBuffer,"Electrons");
  SendMessage(cbCountConvert, CB_ADDSTRING, 0, (LPARAM)(LPSTR)aBuffer);
  wsprintf(aBuffer,"Photons");
  SendMessage(cbCountConvert, CB_ADDSTRING, 0, (LPARAM)(LPSTR)aBuffer);
  wsprintf(aBuffer,"Counts");
  SendMessage(cbCountConvert, CB_SELECTSTRING,0,(LPARAM)(LPSTR)aBuffer);

  // Fill in default wavelength
  wsprintf(aBuffer,"550");
  SendMessage(ebWavelength, WM_SETTEXT, 0, (LPARAM)(LPSTR)aBuffer);
...
void SetSystem(void)
{
  float		fExposure,fAccumTime,fKineticTime,fWavelength;
  int 		errorValue;
  char 		aBuffer[256] = "";
  char 		aBuffer2[256];
  int     mode = 0;
  ...
  fExposure=(float)atof(aBuffer2);
  errorValue = SetExposureTime(fExposure);
  if (errorValue != DRV_SUCCESS) {
    strcat(aBuffer,"Exposure time error\r\n");
  }

  if (caps.ulFeatures & AC_FEATURES_COUNTCONVERT) {
    // Get count convert mode
    GetWindowText(cbCountConvert,aBuffer2,10);
    if(strcmp(aBuffer2,"Electrons")==0) {
      mode = 1;
    }
    else if(strcmp(aBuffer2,"Photons")==0) {
      mode = 2;
    }
    errorValue = SetCountConvertMode(mode);
    if (errorValue != DRV_SUCCESS) {
      strcat(aBuffer,"Count Convert Mode error\r\n");
    }

    // Set Wavelength
    GetWindowText(ebWavelength,aBuffer2, 10);
    fWavelength=(float)atof(aBuffer2);
    errorValue = SetCountConvertWavelength(fWavelength);
    if (errorValue != DRV_SUCCESS) {
      strcat(aBuffer,"Count Convert Wavelength error\r\n");
    }
  }
  else {
    strcat(aBuffer,"Count Convert Mode not supported on this system\r\n");
  }
...
void UpdateDialogWindows(void)
{
  ...
  UpdateWindow(cbCountConvert);
  UpdateWindow(stCountConvert);
  UpdateWindow(ebWavelength);
  UpdateWindow(stWavelength);
  UpdateWindow(ebTemp);
  UpdateWindow(stTemp);
...
void PaintImage(long maxValue, long minValue, int Start)
{
  ...
  if (lvRunning == FALSE) {// if the screensaver is not running, plot data.
    ...
    modrange     = (float)(maxValue - minValue);
