//------------------------------------------------------------------------------
//  PROJECT:		32-bit Driver Example Code ---- Frame Transfer Acquisitions
//
...
//							series acquisition of images using the frame transfer
//							acquisition mode. It will familiarise you with using the
//							Andor MCD driver library.
//------------------------------------------------------------------------------
...
BOOL CreateWindows(void)
{
  ...
  stNoScans=CreateWindow("STATIC","# of scans in series:",
                          WS_CHILD|WS_VISIBLE|SS_LEFT,
                          10,90,250,20,hwnd,0,hInstance,NULL);
  ebNoScans=CreateWindow("EDIT","",
                          WS_CHILD|WS_VISIBLE|WS_BORDER|ES_LEFT,
                          280,90,50,20,hwnd,0,hInstance,NULL);
  stOpenClose=CreateWindow("STATIC","Open / Close Time (msecs):",
    											WS_CHILD|WS_VISIBLE|SS_LEFT,
													10,140,180,20,hwnd,0,hInstance,NULL);
  ...
  stSelScan=CreateWindow("STATIC","Frame to display:",
                          WS_CHILD|WS_VISIBLE|SS_LEFT,
  ...
  if(!errorFlag){
    ...
    switch(acquisitionMode){
      ...
      case 3:
        ...
        break;
      case 6:
      	wsprintf(aInitializeString,"*Frame Transfer");
        break;
      default:
...
void SetWindowsToDefault(char aInitializeString[256])
{
  ...
  strcat(aBuffer,"Frame Transfer Selected\r\n");
  strcat(aBuffer,"Set to Imaging Mode\r\n");
...
void SetSystem(void)
{
  ...
  if(SetupKinetics(aBuffer)){
    ...
    wsprintf(aBuffer,"Error setting Frame Transfer parameters");
  }
...
BOOL SetupKinetics(char *string)
{
  ...
  if(errorValue!=DRV_SUCCESS)
    strcat(aBuffer,"Set number Frames error\r\n");
...
BOOL AcquireImageData(void)
{
  ...
  if(!gblData){                         		// If there is no data the acq has
    ...
    strcat(aBuffer,"Images taken\r\n");
    wsprintf(aBuffer2,"Frame #%d of %d displayed\r\n\r\n",scanNo,noScans);
    strcat(aBuffer,aBuffer2);
...
BOOL DrawLines(int scanNo,long* pMaxDataValue,long* pMinDataValue)
{
  ...
  if(gblData && pImageArray!=NULL){
    ...
    wsprintf(aBuffer,"Now displaying Frame #%d of %d\r\n",scanNo,noScans);
    wsprintf(aBuffer2,"Max data value is %d counts\r\n",MaxValue);
    strcat(aBuffer,aBuffer2);
