//------------------------------------------------------------------------------
//  PROJECT:		32-bit Driver Example Code ---- Spurious Noise Filter
//                                              
//  Copyright 2010. All Rights Reserved
//
//  FILE:				spuriousnoisefilter.c
...
const int MAXNOISETHRESHOLD = 10;
const int MAXCOUNTTHRESHOLD = 0xFFFF;
...
BOOL CreateWindows(void)
{
  ...
  stNoiseFilter=CreateWindow("STATIC","Noise Filter Mode:",
                        WS_CHILD|WS_VISIBLE|SS_LEFT,
                        10,110,160,20,hwnd,0,hInstance,NULL);
  cbNoiseFilter=CreateWindow("COMBOBOX","",
                        WS_CHILD|WS_VISIBLE|CBS_DROPDOWNLIST,
                        180,110,120,180,hwnd,0,hInstance,NULL);
  stThreshold=CreateWindow("STATIC","Threshold:",
                        WS_CHILD|WS_VISIBLE|SS_LEFT,
                        10,140,160,20,hwnd,0,hInstance,NULL);
  ebThreshold=CreateWindow("EDIT","",
                        WS_CHILD|WS_VISIBLE|WS_BORDER|ES_LEFT,
                        250,140,50,20,hwnd,0,hInstance,NULL);
...
void SetWindowsToDefault(char aInitializeString[256])
{
  ...
  // Add Noise Filter options to combobox
  wsprintf(aBuffer,"None");
  SendMessage(cbNoiseFilter, CB_ADDSTRING, 0, (LPARAM)(LPSTR)aBuffer);
  wsprintf(aBuffer,"Median");
  SendMessage(cbNoiseFilter, CB_ADDSTRING, 0, (LPARAM)(LPSTR)aBuffer);
  wsprintf(aBuffer,"Level Above");
  SendMessage(cbNoiseFilter, CB_ADDSTRING, 0, (LPARAM)(LPSTR)aBuffer);
  wsprintf(aBuffer,"Interquartile Range");
  SendMessage(cbNoiseFilter, CB_ADDSTRING,0,(LPARAM)(LPSTR)aBuffer);
  wsprintf(aBuffer,"Noise Threshold");
  SendMessage(cbNoiseFilter, CB_ADDSTRING,0,(LPARAM)(LPSTR)aBuffer);

  // Fill in default threshold
  wsprintf(aBuffer,"2");
  SendMessage(ebThreshold, WM_SETTEXT, 0, (LPARAM)(LPSTR)aBuffer);
...
void SetSystem(void)
{
  ...
  if (caps.ulFeatures & AC_FEATURES_REALTIMESPURIOUSNOISEFILTER) {
    // Get spurious noise filter mode
    GetWindowText(cbNoiseFilter,aBuffer2,10);
    if(strcmp(aBuffer2,"None")==0) {
      mode = 0;
    }
    else if(strcmp(aBuffer2,"Median")==0) {
      mode = 1;
    }
    else if(strcmp(aBuffer2,"Level Above")==0) {
      mode = 2;
    }
    else {
      strcat(aBuffer,"Spurious Noise Filter Mode not supported for realtime acquisition\r\n");
    }

    errorValue = Filter_SetMode(mode);
    if (errorValue != DRV_SUCCESS) {
      strcat(aBuffer,"Spurious Noise Filter Mode error\r\n");
    }
    if (mode > 0) {
      // Set Threshold
      GetWindowText(ebThreshold,aBuffer2, 10);
      iThreshold=atoi(aBuffer2);
      if (iThreshold < 0) {
        iThreshold = 0;
      }
      else if (mode == 2 &&
               iThreshold > MAXCOUNTTHRESHOLD) {
        iThreshold = MAXCOUNTTHRESHOLD;
      }
      else if (mode != 2 &&
               iThreshold > MAXNOISETHRESHOLD) {
        iThreshold = MAXNOISETHRESHOLD;
      }

      errorValue = Filter_SetThreshold(iThreshold);
      if (errorValue != DRV_SUCCESS) {
        strcat(aBuffer,"Spurious Noise Filter Threshold error\r\n");
      }
    }
  }
  else {
    strcat(aBuffer,"Spurious Noise Filter Mode not supported on this system\r\n");
  }
...
void ProcessPushButtons(LPARAM lparam)
{
  ...
  if(lparam==(LPARAM)pbPostProcess){
    if(status!=DRV_ACQUIRING){
      if (pImageArray) {
        if (caps.ulFeatures & AC_FEATURES_POSTPROCESSSPURIOUSNOISEFILTER) {
          char aBuffer[256] = "";
          char aBuffer2[256];
          int iMode, iThreshold;
          // Get spurious noise mode
          GetWindowText(cbNoiseFilter,aBuffer2,20);
          if(strcmp(aBuffer2,"Median")==0) {
            iMode = 1;
          }
          else if(strcmp(aBuffer2,"Level Above")==0) {
            iMode = 2;
          }
          else if(strcmp(aBuffer2,"Interquartile Range")==0) {
            iMode = 3;
          }
          else if(strcmp(aBuffer2,"Noise Threshold")==0) {
            iMode = 4;
          }
          else {
            strcat(aBuffer,"Spurious Noise Filter Mode not supported for realtime acquisition\r\n");
          ...
          errorValue = PostProcessNoiseFilter(pImageArray, pOutputImage, (gblYPixels*gblXPixels), 100, iMode, iThreshold, gblYPixels, gblXPixels);
...
void SwitchCoolerOn(void)
{
  ...
  GetWindowText(ebTemp,aBuffer2, 10);
...
void UpdateDialogWindows(void)
{
  ...
  UpdateWindow(cbNoiseFilter);
  UpdateWindow(stNoiseFilter);
  UpdateWindow(ebThreshold);
  UpdateWindow(stThreshold);
  UpdateWindow(ebTemp);
  UpdateWindow(stTemp);
