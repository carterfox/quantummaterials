//------------------------------------------------------------------------------
//  PROJECT:		32-bit Driver Example Code ---- Multi Track Acquisition
//
...
BOOL CreateWindows(void)
{
  ...
  stNoTracks=CreateWindow("STATIC","# of tracks:",
                          WS_CHILD|WS_VISIBLE|SS_LEFT,
                          20,80,160,20,hwnd,0,hInstance,NULL);
  ebNoTracks=CreateWindow("EDIT","",
                          WS_CHILD|WS_VISIBLE|WS_BORDER|ES_LEFT,
                          270,80,50,20,hwnd,0,hInstance,NULL);
  stTrackHeight=CreateWindow("STATIC","Track Height (in pixels):",
                          WS_CHILD|WS_VISIBLE|SS_LEFT,
                          10,110,160,20,hwnd,0,hInstance,NULL);
  ebTrackHeight=CreateWindow("EDIT","",
                          WS_CHILD|WS_VISIBLE|WS_BORDER|ES_LEFT,
                          270,110,50,20,hwnd,0,hInstance,NULL);
  stTrackOffset=CreateWindow("STATIC","Track Offset:",
                          WS_CHILD|WS_VISIBLE|SS_LEFT,
                          10,140,160,20,hwnd,0,hInstance,NULL);
  ebTrackOffset=CreateWindow("EDIT","",
                          WS_CHILD|WS_VISIBLE|WS_BORDER|ES_LEFT,
...
void SetWindowsToDefault(char aInitializeString[256])
{
  ...
  // Fill in default number of tracks
  wsprintf(aBuffer,"5");
  SendMessage(ebNoTracks, WM_SETTEXT, 0, (LPARAM)(LPSTR)aBuffer);

  // Fill in default track height
  wsprintf(aBuffer,"10");
  SendMessage(ebTrackHeight, WM_SETTEXT, 0, (LPARAM)(LPSTR)aBuffer);

  // Fill in default offset
  wsprintf(aBuffer,"0");
  SendMessage(ebTrackOffset, WM_SETTEXT, 0, (LPARAM)(LPSTR)aBuffer);
  ...
  strcat(aBuffer,"Set to MultiTrack Mode\r\n");
...
void SetSystem(void)
{
  ...
  char 		aBuffer[256];
  char 		aBuffer2[256];
  int 		noTracks,trackHeight,trackOffset,bottom,gap;
  ...
  GetWindowText(ebNoTracks,aBuffer2,10);
  noTracks=atoi(aBuffer2);
  GetWindowText(ebTrackHeight,aBuffer2,10);
  trackHeight=atoi(aBuffer2);
  GetWindowText(ebTrackOffset,aBuffer2,10);
  trackOffset=atoi(aBuffer2);
  // Check to make sure parameters are valid
  if(noTracks>gblYPixels){
  	noTracks=gblYPixels;
    wsprintf(aBuffer2,"%d",noTracks);
    SendMessage(ebNoTracks,WM_SETTEXT,0,(LPARAM)(LPSTR)aBuffer2);
  }
  if(noTracks*trackHeight>gblYPixels){
  	trackHeight=gblYPixels/noTracks;
    wsprintf(aBuffer2,"%d",trackHeight);
    SendMessage(ebTrackHeight,WM_SETTEXT,0,(LPARAM)(LPSTR)aBuffer2);
  }

  SetMultiTrack(noTracks,trackHeight,trackOffset,&bottom,&gap);
  wsprintf(aBuffer2,"Bottom line is row %d\r\nGap between tracks is %d pixels\r\n",bottom,gap);
  strcat(aBuffer,aBuffer2);
...
void UpdateDialogWindows(void)
{
  ...
  UpdateWindow(stNoTracks);
  UpdateWindow(ebNoTracks);
  UpdateWindow(stTrackHeight);
  UpdateWindow(ebTrackHeight);
  UpdateWindow(stTrackOffset);
  UpdateWindow(ebTrackOffset);
...
BOOL AcquireImageData(void)
{
  ...
  if(!gblData){                             // If there is no data the acq has
    ...
    strcat(aBuffer,"Multi-Track Scan taken\r\n");
