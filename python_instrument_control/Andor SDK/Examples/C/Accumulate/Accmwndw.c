...
BOOL CreateWindows(void)
{
  ...
  stExposure=CreateWindow("STATIC","Exposure time (secs):",
                        WS_CHILD|WS_VISIBLE|SS_LEFT,
                        10,120,160,20,hwnd,0,hInstance,NULL);
  ...
  stAccums=CreateWindow("STATIC","No of Accums",
                        WS_CHILD|WS_VISIBLE|SS_LEFT,
                        10,70,160,20,hwnd,0,hInstance,NULL);
  ebAccums=CreateWindow("EDIT","",
                        WS_CHILD|WS_VISIBLE|WS_BORDER|ES_LEFT,
                        270,70,50,20,hwnd,0,hInstance,NULL);
  stAccumTime=CreateWindow("STATIC","Accumulation cycle time",
                        WS_CHILD|WS_VISIBLE|SS_LEFT,
                        10,90,160,20,hwnd,0,hInstance,NULL);
  ebAccumTime=CreateWindow("EDIT","",
                        WS_CHILD|WS_VISIBLE|WS_BORDER|ES_LEFT,
                        270,90,50,20,hwnd,0,hInstance,NULL);
...
void SetWindowsToDefault(char aInitializeString[256])
{
  ...
  SendMessage(ebExposure, WM_SETTEXT, 0, (LPARAM)(LPSTR)aBuffer);
     ...
     // Fill in default accum cycle time
  wsprintf(aBuffer,"1");
  SendMessage(ebAccumTime, WM_SETTEXT, 0, (LPARAM)(LPSTR)aBuffer);

    // Fill in default no of accums time
  wsprintf(aBuffer,"5");
  SendMessage(ebAccums, WM_SETTEXT, 0, (LPARAM)(LPSTR)aBuffer);
...
void SetSystem(void)
{
  ...
  GetWindowText(ebAccumTime,aBuffer2,10);
  accumcyc=atof(aBuffer2);
  errorValue = SetAccumulationCycleTime(accumcyc);
  if (errorValue != DRV_SUCCESS)
    wsprintf(aBuffer,"SetAccumulationCycleTime error\r\n");

  GetAcquisitionTimings(&exposure, &accumulate, &kinetic);


    sprintf(mybuf,"%.2f",accumulate);
  SendMessage(ebAccumTime, WM_SETTEXT, 0, (LPARAM)(LPSTR)mybuf);

    // Set No of accums
  GetWindowText(ebAccums,aBuffer2,10);
  noaccums=atol(aBuffer2);
  errorValue = SetNumberAccumulations(noaccums);
  if (errorValue != DRV_SUCCESS)
    wsprintf(aBuffer,"Number accumulation error\r\n");
...
void UpdateDialogWindows(void)
{
  ...
  UpdateWindow(stAccums);
  UpdateWindow(ebAccums);
  UpdateWindow(stAccumTime);
  UpdateWindow(ebAccumTime);
}
