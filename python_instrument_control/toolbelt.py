...
def make_rmcd_saving_file(filename,experiment):
    ...
    elif experiment == 'Esweep-dual':
        header = "#Vb_set(V) Vb(V) Ib(nA) R(V) R_std(V) thetaR(deg) thetaR_std(deg) dR(V) dR_std(V) thetadR(deg) thetadR_std(deg) Vt_set(V) Vt(V) It(nA)"
